...
async def main() -> None:
    ...
    async with websockets.serve(
        ...
        logger.info(f"HTTP: {scheme_http}://localhost:{HTTP_PORT}/")
        logger.info(f"WS:   {scheme_ws}://localhost:{WS_PORT}/ws")
        if scheme_http == "http":
            logger.info("iOS Safari 通常需要 HTTPS 才能使用麦克风。")
            logger.info("可生成自签名证书并启动：")
            logger.info('  openssl req -x509 -newkey rsa:2048 -keyout key.pem -out cert.pem -days 365 -nodes -subj "/CN=localhost"')
            logger.info("  export SSL_CERTFILE=cert.pem SSL_KEYFILE=key.pem")
