...
def test_bge_search(query, is_qa:bool=False):
    if is_qa:
        qa_results = es_handler.search_qa_question(f"alpha_test_qa", query, top_k=40,embed_model_type="bge_code", embed_model=model)
        ...
        search_results = es_handler.search_by_vector(f"alpha_test_qa", query, top_k=5, embedding_model_type="bge_code", embed_model=model)
        context = "\n".join([f"内容: {hit['_source']['content']}" for hit in search_results])
        print(context)

def test_doubao_search(query, index_name="prod_qa"):
    qa_results = es_handler.search_qa_question(index_name, query, top_k=40)
    # qa_questions = [{"question": hit['_source']['question'], "answer": hit['_source']['answer'], "score": hit["score"]} for hit in qa_results]
    qa_questions = [{"question": hit['_source']['question'], "score": hit["_score"]} for hit in qa_results]
    print(f"doubao search qa res: {qa_questions}")
    return qa_results

def test_doubao_answer_search(query, index_name="alpha_test_qa"):
    qa_results = es_handler.search_qa_by_answer(index_name, query, top_k=3)
    # qa_questions = [{"question": hit['_source']['question'], "answer": hit['_source']['answer'], "score": hit["score"]} for hit in qa_results]
